...
void main() {
  ...
  setUp(() {
    ...
    bookmarkLocalDatasource =
        IBookmarkLocalDatasource(databaseHelper: mockDatabaseHelper);
  });
  ...
  group('BookmarkLocalDatasource', () {
    group('insertBookmark', () {
      test('should return success message when insert data is successful',
          () async {
        when(mockDatabaseHelper.insertBookmark(tBookmarkTable))
            .thenAnswer((_) async => 1);

        final result =
            await bookmarkLocalDatasource.insertBookmark(tBookmarkTable);
    ...
    group('removeBookmark', () {
      test('should return success message when remove data is successful',
          () async {
        when(mockDatabaseHelper.removeBookmark(tBookmarkTable))
            .thenAnswer((_) async => 1);

        final result =
            await bookmarkLocalDatasource.removeBookmark(tBookmarkTable);
    ...
    group('getBookmarks', () {
      test('should return list of BookmarkTable when get data is successful',
          () async {
