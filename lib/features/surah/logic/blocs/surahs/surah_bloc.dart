...
class SurahBloc extends Bloc<SurahEvent, SurahState> {
  ...
  void _onGetSurahs(OnGetSurah event, Emitter<SurahState> emit) async {
    emit(const SurahLoading());
    final result = await getSurahsUseCase();
    result.fold((failure) => emit(SurahError(failure.message)), (surahs) {
      emit(SurahLoaded(surahs.data));
    });
  }

  void _onSearchSurah(OnSearchSurah event, Emitter<SurahState> emit) async {
    emit(const SurahLoading());
